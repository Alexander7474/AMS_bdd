	...
	public static void loadMainApplication(String username) {
		...
		suppliers.addActionListener(e -> {
			...
			String[] columnNames = { "SIRET", "Nom", "Adresse", "Téléphone", "Email" }; // Colonnes du tableau des //
																						// fournisseurs
			Object[][] data = {}; // Données sur les fournisseurs
			...
			try {
				...
				while (rs.next()) {
					rows.add(new Object[] { rs.getString("siret"), rs.getString("nom"), rs.getString("adresse"),
							rs.getString("numero_tel"), rs.getString("email") });
				}
			...
			JTable suppliersTable = new JTable(data, columnNames);
			...
			addSupplierButton.addActionListener(addEvent -> {
				...
				if (result == JOptionPane.OK_OPTION) {
					try {
						...
						PreparedStatement statement = Connexion.getConnexion()
								.prepareStatement("INSERT INTO fournisseur " + fournisseur.getValues());
						fournisseur.composeStatement(statement);
